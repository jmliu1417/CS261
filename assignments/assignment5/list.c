 ...
 *     passed are to be considered equal, thid function should return 0.
...
void list_remove(struct list* list, void* val, int (*convert)(void*)) {
  ...
  while (curr) {
     ...
     * the list.
     */
    struct ht_node* elem = curr->val;

    if (convert(elem->key) == convert(val)) {
      if (prev) {
        prev->next = curr->next;
      } else {
        list->head = curr->next;
...
/*
This function returns 0 if list is not empty 1 if llist is empty
 */
...
/*
this function finds a value in a list based off a key
 *
 * Params:
 *   list - the linked list to be reversed.  May not be NULL.  When this
 *     function returns this should contain the reversed list.
 * key - desired key
 * conver - function pointer
 */

void* list_find(struct list* list, void* key, int (*convert)(void*)){
  struct node* curr = list->head;

  while(curr){
    struct ht_node* elem = curr->val;

//check if converted values are equal
    if(convert(elem->key) == convert(key)){
      return elem;
    }
    curr = curr->next;;
  }
