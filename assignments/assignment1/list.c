...
struct list* list_create()
{
    ...
    //allocating memory for the entire linked list
    struct list* linked_list = malloc(sizeof(struct list));

    //establishing linked_list as null
    if (linked_list == NULL) {
        return NULL;
    }

    //setting head to null, don't need to initialize more memory for heads rn
...
void list_free(struct list* list)
{
   ...
   //temp tracker to clear every node
    struct node * current = list->head;
    struct node* temp;

    //cycling through each node in the list
        while(current != NULL){
            temp = current->next;
            free(current);
            current = temp;
    ...
    //free the whole list in reverse (freed the nodes now just the list)
    free(list);
    return;
}
...
{
    //creating a completely new node
...
void list_insert_end(struct list* list, void* val)
{
    ...
    if(list->head == NULL){
        ...
        //cycle through each node till the last one
        while(current->next != NULL){
            current = current->next;
        }
        //assign last node to new_node
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    //cycle until we set current to null
    while(current != NULL){

//comparing values per node
        if(cmp(val, current->val) == 0){
            //beginning value to remove
            if(previous == NULL){
                list->head = current->next;
                free(current);
            }else{
                //set prev next to the next over value
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
 {
    //setting initial to 0 to inc later
    int position = 0;

    //setting up current
    struct node * current = list->head;

    //counting every time values are the same
    while(current != NULL){
        if(cmp(val, current->val) == 0){
            return position;
        }
...
void list_reverse(struct list* list)
{
    ...
    //keeping track of current, prev, and next nodes to flip the directions
    struct node * current = list->head;
    struct node * previous = NULL;
    ...
    while(current != NULL){
        //next going to next value
        next = current->next;
        //flipping current to point to previous
