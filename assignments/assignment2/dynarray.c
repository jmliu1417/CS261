...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
//adding the buffer via that strange formula
...
void dynarray_remove(struct dynarray* da, int idx) {
  ...
  for (int i = idx; i < da->size - 1; i++) {
    //again making that buffer thing
    da->data[(da->start + i) % da->capacity] = da->data[(da->start + i+1) % da->capacity];
  }
...
/*
 *functiton checks if da is empty
 *
 * Params:
 *   da - the dynamic array from which to get a value.  May not be NULL.
 *   
 */
int da_is_empty(struct dynarray* da){ 
    if(da->size == 0){
      return 1;
    }
