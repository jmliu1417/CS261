 ...
 * Specifically, it should return 1sif the specified queue is empty (i.e.
...
int queue_isempty(struct queue* queue) {
	...
	//checking if its emptyyy
	if(da_is_empty(queue->array) == 1){
		return 1;
	}
...
void queue_enqueue(struct queue* queue, void* val) {
	 ...
	 * FIXME:
	 */
//edited insert to create the buffer
...
void* queue_dequeue(struct queue* queue) {
	 ...
	 * FIXME:
	 */
	//setting data to front of queued
